#!/usr/bin/env python3
"""Live deploy-architecture session manager."""
...
def session_log_path(session_dir: Path) -> Path:
    ...
    return Path(tempfile.gettempdir()) / f"deploy-architecture-{session_key}-{SESSION_LOG_NAME}"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Manage a live deploy architecture session")
    ...
    start.add_argument("--port", type=int, default=8766, help="Preferred local port for the live dashboard server")
    start.add_argument(
        "--open-browser",
...
def build_live_payload_bundle(
    ...
    meta_payload = {
        "title": config.get("title", "Deploy Architecture Live Session"),
        "prompt": prompt,
...
def render_session_html(
    ...
    builder.render_dashboard(
        ...
        config.get("title", "Deploy Architecture Live Session"),
        {"nodes": [], "relations": []},
        payload["appConfig"],
        bundle["skill_state"],
        html_path,
