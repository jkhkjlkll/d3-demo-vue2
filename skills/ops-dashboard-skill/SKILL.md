...
description: 通过本地脚本和内置 HTML 模板渲染运维图谱看板。这个 skill 只接受 agent 先调用 MCP 后提供的本地 JSON 输入，优先直接使用 MCP 返回的原始文件路径；不允许直连后端 API，也不允许 mock 回退。
---
...
- 如果 `query_ges` 已经直接返回符合 schema 的 JSON 文本，不要再二次拼装、pretty-print 或 PowerShell 重写
- 之后必须执行 `python3 scripts/run_ops_dashboard.py --input-json "<MCP原始文件路径或./runtime/mcp-input.json>" --prompt "..."`
...
- 不要再复制一份到 `./runtime/mcp-input.json`
...
1. 如果 MCP 已返回一个宿主可直接读取的本地文件，就直接使用那个文件路径
2. 只有没有可读文件路径、只有原始 JSON 文本时，才使用下面这个固定相对路径：

```text
./runtime/mcp-input.json
```

如果使用的是固定相对路径，而宿主确实需要绝对路径，请在运行时基于当前 skill 目录解析这个相对路径；不要在文档或 prompt 里写死某台机器上的仓库绝对路径。

只有在“写固定路径”这个分支里，如果宿主 file tool 对“覆盖已有文件”有限制，请按下面处理：
...
- 每次 update 都会重新读取当前配置里的 `input_json`
...
- `nodes[].lifecycle_state` 只作为原始扩展属性保留，不参与页面 health、颜色、筛选或统计
- 页面上的资源 health 只看该资源是否通过 `relations` 关联到 `resource_type == "alarm"` 的节点：
  - 有关联 alarm：`告警`
  - 没有关联 alarm：`正常`
- `alarm` 节点继续展示，但不显示 health
- `relations` 里的 `health_status / health_level / health_text` 只保留空字段做兼容，不再有业务含义
