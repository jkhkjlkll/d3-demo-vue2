...
- MCP 返回必须先写入本地 JSON 文件，再执行本 skill
- 如果宿主 file tool 允许覆盖写，默认可写入 `./runtime/mcp-input.json`
- 如果宿主 file tool 要求“先 read 才能 overwrite”，必须改为写入一个全新的输入文件路径
- 之后必须执行 `python3 scripts/run_ops_dashboard.py --input-json <刚写入的 JSON 路径> --prompt "..."`
...
如果宿主 file tool 对“覆盖已有文件”有限制，推荐先生成一个新的输入路径：
...
然后把 MCP 返回写入这个新文件。这样可以避免 `you must read file ... before overwriting it` 这一类覆盖写限制。
...
- 每次 update 都会重新读取 `--input-json` 指定的最新 JSON
...
这个脚本只负责打印一个可写的新路径，不会触发 MCP，也不会写入内容。
